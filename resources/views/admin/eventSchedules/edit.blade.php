...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.eventSchedule.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.event-schedules.update", [$eventSchedule->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('event_program_date') ? 'is-invalid' : '' }}" name="event_program_date_id" id="event_program_date_id" required>
                    @foreach($event_program_dates as $id => $entry)
                        <option value="{{ $id }}" {{ (old('event_program_date_id') ? old('event_program_date_id') : $eventSchedule->event_program_date->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time_start') ? 'is-invalid' : '' }}" type="text" name="time_start" id="time_start" value="{{ old('time_start', $eventSchedule->time_start) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time_end') ? 'is-invalid' : '' }}" type="text" name="time_end" id="time_end" value="{{ old('time_end', $eventSchedule->time_end) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('topic') ? 'is-invalid' : '' }}" type="text" name="topic" id="topic" value="{{ old('topic', $eventSchedule->topic) }}" required>
                @if($errors->has('topic'))
                    <div class="invalid-feedback">
                        {{ $errors->first('topic') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $eventSchedule->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('speakers') ? 'is-invalid' : '' }}" name="speakers[]" id="speakers" multiple>
                    @foreach($speakers as $id => $speaker)
                        <option value="{{ $id }}" {{ (in_array($id, old('speakers', [])) || $eventSchedule->speakers->contains($id)) ? 'selected' : '' }}>{{ $speaker }}</option>
                    @endforeach
                </select>
